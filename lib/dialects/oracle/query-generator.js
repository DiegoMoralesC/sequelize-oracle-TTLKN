  ...
  var QueryGenerator = {
    ...
    createTableQuery: function (tableName, attributes, options) {
      ...
      if (autoIncrementKeys.length > 0) {
         const uuid =  require("node-uuid").v4().slice(0, 8);
        for (var ikey in autoIncrementKeys) {

          sequences += '\n\n' + Utils._.template(sequenceTpl)({
            sequence: this.quoteIdentifier(tableName.slice(0,10) + '_ID'+ uuid+'_SEQ')
          }).trim();

          triggers += '\n\n' + Utils._.template(triggerTpl)({
            trigger: this.quoteIdentifier(tableName.slice(0,10) + '_ID'+ uuid+'_TRG'),

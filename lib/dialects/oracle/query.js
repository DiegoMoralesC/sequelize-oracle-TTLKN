...
module.exports = (function () {
  var Query = function (connection, sequelize, options) {
    ...
    this.outFormat =
      options.outFormat || this.sequelize.connectionManager.lib.OBJECT;
    ...
    var promise = new Utils.Promise(function (resolve, reject) {
      if (
        ...
        if (
          ...
          self.options.transaction.options.autoCommitTransactionalOFF
        ) {
        ...
        var idTransaction =
          self.options.transaction && self.options.transaction.id
            ? self.options.transaction.id
            : self.uuid;

        var start = new Date();
        var hrstart = process.hrtime();
        self.sequelize.log(
          `[START Execution transaction(${idTransaction}) time]: `,
          now()
        );
        ...
        self.connection.execute(
          ...
          self.options.bind,
          {
            ...
            autoCommit:
              self.connection.autoCommit == null || self.connection.autoCommit,
          },
          function (err, results, fields) {
            ...
            self.sequelize.log(
              `[END Execution transaction(${idTransaction}) time]: %dms`,
              end
            );
            self.sequelize.log(
              `[END Execution transaction(${idTransaction}) time (hr)]: %ds %dms`,
              hrend[0],
              hrend[1] / 1000000
            );
            self.sequelize.log(
              `[END Execution transaction(${idTransaction}) time]: `,
              now()
            );
  ...
  Query.prototype.formatResults = function (data, done) {
    ...
    if (this.isInsertQuery(data)) {
      ...
      if (
        ...
        data &&
        data.outBinds &&
        data.outBinds[this.getInsertIdField()] &&
        data.outBinds[this.getInsertIdField()][0]
      ) {
  ...
  AbstractQuery.prototype.handleInsertQuery = function (results, metaData) {
    if (this.instance) {
      ...
      if (
        results &&
        results.outBinds &&
        results.outBinds[this.getInsertIdField()] &&
        results.outBinds[this.getInsertIdField()][0]
      ) {
